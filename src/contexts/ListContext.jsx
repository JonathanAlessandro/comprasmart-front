...
export const ListProvider = ({ children }) => {
  ...
  const updateItemPrice = async (listId, itemId, price) => {
    const safePrice = price === "" ? null : Number(String(price).replace(",", "."));
    const { data } = await api.patch(`/api/lists/${listId}/items/${itemId}`, {
      price: safePrice,
    });
    setLists((previousLists) => replaceList(previousLists, data));
  ...
  const updateItem = async (listId, itemId, updates) => {
    const { data } = await api.patch(`/api/lists/${listId}/items/${itemId}`, updates);
    setLists((previousLists) => replaceList(previousLists, data));
    return data;
  };
